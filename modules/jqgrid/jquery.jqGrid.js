            ...
            var QueryObject = function(d, q) {
                ...
                this._getOrder = function(data, by, dir, type, dfmt) {
                    ...
                    if (type === 'float' || type === 'number' || type === 'currency' || type === 'numeric') {
                        ...
                        findSortKey = function($cell) {
                            return $.jgrid.parseDate(dfmt, $cell)
                                .getTime();
                        };
                ...
                this._compareValues = function(func, f, v, how, t) {
                    ...
                    if (v !== null) {
                        switch (swst) {
                            ...
                            case 'date':
                            case 'datetime':
                                val = String($.jgrid.parseDate(t.newfmt || 'Y-m-d', val)
                                    .getTime());
                ...
                this.startsWith = function(f, v) {
                    var val = (v == null) ? f : v,
                        length = _trim ? $.trim(val.toString())
                        .length : val.toString()
                        .length;
                    if (_useProperties) {
                        self._append(self._getStr('jQuery.jgrid.getAccessor(this,\'' + f + '\')') + '.substr(0,' + length + ') == ' + self._getStr('"' + self._toStr(v) + '"'));
                    } else {
                        length = _trim ? $.trim(v.toString())
                            .length : v.toString()
                            .length;
                ...
                this.orderBy = function(by, dir, stype, dfmt) {
                    dir = dir == null ? "a" : $.trim(dir.toString()
                        .toLowerCase());
                    if (stype == null) {
                        stype = "text";
                    }
    ...
    $.fn.jqGrid = function(pin) {
        ...
        return this.each(function() {
            ...
            var p = $.extend(true, {
                ...
                lastsort: 0,
                selrow: null,
                onColumnChooserDone: null, //Liveapp v4 
            ...
            var ts = this,
                grid = {
                    ...
                    dragStart: function(i, x, y) {
                        ...
                        $(ts)
                            .triggerHandler("jqGridResizeStart", [x, i]);
                    ...
                    dragEnd: function() {
                        ...
                        if (this.resizing) {
                            ...
                            if (p.forceFit === true) {
                                ...
                                $('table:first', this.bDiv)
                                    .css("width", p.tblwidth + "px");
                                $('table:first', this.hDiv)
                                    .css("width", p.tblwidth + "px");
                            ...
                            $(ts)
                                .triggerHandler("jqGridResizeStop", [nw, idx]);
                    ...
                    populateVisible: function() {
                        ...
                        if (table[0].rows.length) {
                            try {
                                rows = table[0].rows[1];
                                rh = rows ? $(rows)
                                    .outerHeight() || grid.prevRowHeight : grid.prevRowHeight;
                            } catch (pv) {
                        ...
                        var ttop = Math.round(table.position()
                            .top) - scrollTop;
                            ...
                            restoreSelection = function() {
                                ...
                                if (p.multiselect && sra && sra.length > 0) {
                                    for (i = 0; i < sra.length; i++) {
                                        if (sra[i] !== sr) {
                                            $(ts)
                                                .jqGrid("setSelection", sra[i], false, null);
                                        }
                                    }
                                }
                                if (sr) {
                                    $(ts)
                                        .jqGrid("setSelection", sr, false, null);
                                }
                                ts.grid.bDiv.scrollLeft = left;
                                $(ts)
                                    .unbind('.selectionPreserver', restoreSelection);
                            };
                        $(ts)
                            .bind('jqGridGridComplete.selectionPreserver', restoreSelection);
                    }
                };
            ...
            $("<div class='ui-widget-overlay jqgrid-overlay' id='lui_" + this.id + "'></div>")
                .insertBefore(gv);
            $("<div class='loading ui-state-default ui-state-active' id='load_" + this.id + "'>" + this.p.loadtext + "</div>")
                .insertBefore(gv);
            $(this)
                .attr({
                    cellspacing: "0",
            ...
            var sortkeys = ["shiftKey", "altKey", "ctrlKey"],
                ...
                formatCol = function(pos, rowInd, tv, rawObject, rowId, rdata) {
                    ...
                    if (rowInd === 0) {
                        ...
                        if (celp && typeof celp === "string") {
                            ...
                            acp = celp.replace('-style', '-sti')
                                .split(/style/);
                            if (acp.length === 2) {
                                acp[1] = $.trim(acp[1].replace('-sti', '-style')
                                    .replace("=", ""));
                ...
                constructTr = function(id, hide, altClass, rd, cur, selected) {
                    var tabindex = '-1',
                        ...
                        rowAttrObj = $(ts)
                        .triggerHandler("jqGridRowAttr", [rd, cur, id]);
                ...
                addXmlData = function(xml, t, rcnt, more, adjust) {
                    ...
                    if ($.isFunction(xmlRd.userdata)) {
                        ...
                        $.jgrid.getXmlData(xml, xmlRd.userdata, true)
                            .each(function() {
                                ts.p.userData[this.getAttribute("name")] = $(this)
                                    .text();
                            });
                    }
                ...
                addLocalData = function() {
                    ...
                    if (ts.p.treeGrid) {
                        $(ts)
                            .jqGrid("SortTree", st, ts.p.sortorder, cmtypes[st].stype || 'text', cmtypes[st].srcfmt || '');
                        return;
                    }
                    var compareFnMap = {
                            ...
                            'nc': function(queryObj, op) {
                                return op === "OR" ? queryObj.orNot()
                                    .contains : queryObj.andNot()
                                    .contains;
                            },
                            ...
                            'bn': function(queryObj, op) {
                                return op === "OR" ? queryObj.orNot()
                                    .startsWith : queryObj.andNot()
                                    .startsWith;
                            },
                            'en': function(queryObj, op) {
                                return op === "OR" ? queryObj.orNot()
                                    .endsWith : queryObj.andNot()
                                    .endsWith;
                            },
                    ...
                    function tojLinq(group) {
                        ...
                        if (group.groups != null) {
                            gor = group.groups.length && group.groupOp.toString()
                                .toUpperCase() === "OR";
                            if (gor) {
                                query.orBegin();
                            }
                        ...
                        if (group.rules != null) {
                            ...
                            //}
                            try {
                                ror = group.rules.length && group.groupOp.toString()
                                    .toUpperCase() === "OR";
                                if (ror) {
                                    query.orBegin();
                                }
                ...
                updatepager = function(rn, dnd) {
                    ...
                    if (pgboxes) {
                        ...
                        if (ts.p.pginput === true) {
                            ...
                            $(sppg)
                                .html($.fmatter ? $.fmatter.util.NumberFormat(ts.p.lastpage, fmt) : ts.p.lastpage);
                        ...
                        if (ts.p.viewrecords) {
                            if (ts.p.reccount === 0) {
                                ...
                                $(".ui-paging-info", pgboxes)
                                    .html($.jgrid.format(ts.p.recordtext, from, to, tot));
                            }
                        }
                    ...
                    if (!ts.grid.hDiv.loading) {
                        ...
                        switch (dt) {
                            ...
                            case "script":
                                $.ajax($.extend({
                                    ...
                                    success: function(data, st, xhr) {
                                        ...
                                        $(ts)
                                            .triggerHandler("jqGridLoadComplete", [data]);
                                        if (lc) {
                                            lc.call(ts, data);
                                        }
                                        $(ts)
                                            .triggerHandler("jqGridAfterLoadComplete", [data]);
                                        if (pvis) {
                            ...
                            case "xmlstring":
                                ...
                                $(ts)
                                    .triggerHandler("jqGridLoadComplete", [dstr]);
                                if (lcf) {
                                    ts.p.loadComplete.call(ts, dstr);
                                }
                                $(ts)
                                    .triggerHandler("jqGridAfterLoadComplete", [dstr]);
                                ts.p.datatype = "local";
                                ts.p.datastr = null;
                                endReq();
                                break;
                            ...
                            case "clientside":
                                ...
                                $(ts)
                                    .triggerHandler("jqGridLoadComplete", [req]);
                                if (lc) {
                                    lc.call(ts, req);
                                }
                                $(ts)
                                    .triggerHandler("jqGridAfterLoadComplete", [req]);
                                if (pvis) {
                                    ts.grid.populateVisible();
                                }
                                endReq();
                ...
                setPager = function(pgid, tp) {
                    ...
                    if (ts.p.viewrecords === true) {
                        $("td#" + pgid + "_" + ts.p.recordpos, "#" + pgcnt)
                            .append("<div dir='" + dir + "' style='text-align:" + ts.p.recordpos + "' class='ui-paging-info'></div>");
                    }
                    ...
                    tdw = $(".ui-jqgrid")
                        .css("font-size") || "11px";
                    $(document.body)
                        .append("<div id='testpg' class='ui-jqgrid ui-widget ui-widget-content' style='font-size:" + tdw + ";visibility:hidden;' ></div>");
                    twd = $(pgl)
                        .clone()
                        .appendTo("#testpg")
                        .width();
                    ...
                    $('.ui-pg-selbox', "#" + pgcnt)
                        .bind('change', function() {
                            ...
                            if (ts.p.pager) {
                                $('.ui-pg-selbox', ts.p.pager)
                                    .val(this.value);
                            }
                            if (ts.p.toppager) {
                                $('.ui-pg-selbox', ts.p.toppager)
                                    .val(this.value);
                            }
                            populate();
                            return false;
                    ...
                    if (ts.p.pgbuttons === true) {
                        ...
                        $("#first" + $.jgrid.jqID(tp) + ", #prev" + $.jgrid.jqID(tp) + ", #next" + $.jgrid.jqID(tp) + ", #last" + $.jgrid.jqID(tp))
                            .click(function() {
                ...
                sortData = function(index, idxcol, reload, sor, obj) {
                    ...
                    if (ts.p.multiSort) {
                        ...
                        if (!ts.p.viewsortcols[0]) {
                            if (ts.p.lastsort !== idxcol) {
                                if (ts.p.frozenColumns) {
                                    ts.grid.fhDiv.find("span.s-ico")
                                        .hide();
                                }
                                $("span.s-ico", previousSelectedTh)
                                    .hide();
                                $("span.s-ico", newSelectedTh)
                                    .show();
                            }
                        }
                    ...
                    if ($(ts)
                        .triggerHandler("jqGridSortCol", [ts.p.sortname, idxcol, ts.p.sortorder]) === 'stop') {
                ...
                getOffset = function(iCol) {
                    ...
                    ret.push($(ts.grid.hDiv)
                        .position()
                        .top);
                    ret.push($(ts.grid.bDiv)
                        .offset()
                        .top - $(ts.grid.hDiv)
                        .offset()
                        .top + $(ts.grid.bDiv)
            ...
            if (this.p.multiselect) {
                ...
                $('#cb_' + $.jgrid.jqID(ts.p.id), this)
                    .bind('click', function() {
                        ...
                        if (this.checked) {
                            $(ts.rows)
                                .each(function(i) {
                                    if (i > 0) {
                                        if (!$(this)
                                            .hasClass("ui-subgrid") && !$(this)
                                            .hasClass("jqgroup") && !$(this)
                                            .hasClass('ui-state-disabled')) {
                        ...
                        $(ts)
                            .triggerHandler("jqGridSelectAll", [chk ? ts.p.selarrrow : emp, chk]);
            ...
            $(gv)
                .css("width", grid.width + "px");
            thead = $("thead:first", ts)
                .get(0);
            var tfoot = "";
            if (ts.p.footerrow) {
                tfoot += "<table role='grid' style='width:" + ts.p.tblwidth + "px' class='ui-jqgrid-ftable' cellspacing='0' cellpadding='0' border='0'><tbody><tr role='row' class='ui-widget-content footrow footrow-" + dir + " ui-state-default'>";
            } //Liveapp v4
            ...
            $("th", thr)
                        ...
                        if (ts.p.multiSort) {
                            if (ts.p.viewsortcols[0]) {
                                ...
                                if (ts.p.colModel[j].lso) {
                                    $("div span.ui-icon-" + ts.p.colModel[j].lso, this)
                                        .removeClass("ui-state-disabled");
                                }
                ...
                .mousedown(function(e) {
                    if ($(e.target)
                        .closest("th>span.ui-jqgrid-resize")
                        .length !== 1) {
                        return;
                    }
                ...
                .bind("contextmenu", function() { //Liveapp v4
                    $(ts)
                        .jqGrid('columnChooser', {
                            done: function() {
                                if ($.isFunction(ts.p.onColumnChooserDone)) {
                                    ts.p.onColumnChooserDone.call();
                                }
                            }
                        });
                    return false;
            ...
            $(this)
                ...
                .append(firstr);
            firstr = null;
            var hTable = $("<table class='ui-jqgrid-htable' style='width:" + ts.p.tblwidth + "px' role='grid' aria-labelledby='gbox_" + this.id + "' cellspacing='0' cellpadding='0' border='0'></table>")
                .append(thead),
            ...
            $(ts)
                ...
                .click(function(e) {
                    ...
                    ptr = $(td, ts.rows)
                        .closest("tr.jqgrow");
                    if ($(ptr)
                        .length === 0 || ptr[0].className.indexOf('ui-state-disabled') > -1 || ($(td, ts)
                    ...
                    var scb = $(td)
                        .hasClass("cbox"),
                        cSel = $(ts)
                        .triggerHandler("jqGridBeforeSelectRow", [ptr[0].id, e]);
                    ...
                    if (cSel === true) {
                        ...
                        tdHtml = $(td)
                            ...
                            .html();
                        $(ts)
                            .triggerHandler("jqGridCellSelect", [ri, ci, tdHtml, e]);
                        ...
                        if (ts.p.cellEdit === true) {
                            if (ts.p.multiselect && scb) {
                                $(ts)
                                    .jqGrid("setSelection", ri, true, e);
                            } else {
                                ri = ptr[0].rowIndex;
                                try {
                                    $(ts)
                                        .jqGrid("editCell", ri, ci, true);
                                } catch (_) {}
                            }
                            ...
                            if (e[ts.p.multikey]) {
                                ...
                                scb = $("#jqg_" + $.jgrid.jqID(ts.p.id) + "_" + ri)
                                    .is(":checked");
                ...
                .dblclick(function(e) {
                    ...
                    ci = $.jgrid.getCellIndex(td);
                    $(ts)
                        .triggerHandler("jqGridDblClickRow", [$(ptr)
                            .attr("id"), ri, ci, e
                        ]);
            ...
            $(grid.bDiv)
                .append($('<div style="position:relative;' + (isMSIE && $.jgrid.msiever() < 8 ? "height:0.01%;" : "") + '"></div>')
                    .append('<div></div>')
                    .append(this))
            ...
            var arf = ts.p.hidegrid === true ? $("<a role='link' class='ui-jqgrid-titlebar-close HeaderButton' />")
                .hover(
                    function() {
                ...
                if (ts.p.hidegrid === true) {
                    $(".ui-jqgrid-titlebar-close", grid.cDiv)
                        .click(function(e) {
                            ...
                            counter = $(elems, "#gview_" + $.jgrid.jqID(ts.p.id))
                                .length;
                                    ...
                                    .slideUp("fast", function() {
                                        ...
                                        if (counter === 0) {
                                            ...
                                            if ($("#gbox_" + $.jgrid.jqID(ts.p.id))
                                                ...
                                                $(".ui-resizable-handle", "#gbox_" + $.jgrid.jqID(ts.p.id))
                                                    .hide();
                                            }
                                            $(ts)
                                                .triggerHandler("jqGridHeaderClick", [ts.p.gridstate, e]);
                                            if (onHdCl) {
                                                if (!hg) {
            ...
            $(document)
                .bind("mouseup.jqGrid" + ts.p.id, function() {
                    if (grid.resizing) {
                        grid.dragEnd();
                        return false;
                    }
    ...
    $.jgrid.extend({
        ...
        setSelection: function(selection, onsr, e) {
            return this.each(function() {
                ...
                pt = $($t)
                    .jqGrid('getGridRowById', selection);
                ...
                if (!$t.p.multiselect) {
                    if (pt.className !== "ui-subgrid") {
                        if ($t.p.selrow !== pt.id && true == false) { //Liveapp v4                          
                            $($($t)
                                    .jqGrid('getGridRowById', $t.p.selrow))
                                .removeClass("ui-state-highlight")
                                .attr({
                                    "aria-selected": "false",
                                    "tabindex": "-1"
                        ...
                        if (onsr) {
                            $($t)
                                .triggerHandler("jqGridSelectRow", [pt.id, stat, e]);
                            if ($t.p.onSelectRow) {
                            ...
                            .each(function(i) {
                                ...
                                if (nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
                                    if ($t.p.treeGrid === true && nm === $t.p.ExpandColumn) {
                                        res[nm] = $.jgrid.htmlDecode($("span:first", this)
                                            .html());
                                    } else {
                                        try {
                        ...
                        $(this.p.colModel)
                            .each(function(i) {
                                ...
                                if (dval !== undefined) {
                                    ...
                                    vl = t.formatter(rowid, dval, i, data, 'set'); //Liveapp v4
                                    title = this.title ? {
                                    ...
                                    if (t.p.treeGrid === true && nm === t.p.ExpandColumn) {
                                        ...
                                        //Liveapp v4                                     
                                        var selector = $("td[role='gridcell']:eq(" + i + ")", ind);
                                        var editor = js.findInput(selector);
                                        if (!editor) {
                                            selector.html(vl)
                                                .attr(title);
                                        } else {
            ...
            if (rdata) {
                ...
                this.each(function() {
                    ...
                    while (k < datalen) {
                        ...
                        if (t.p.subGrid === true) {
                            $(t)
                                .jqGrid("addSubGrid", gi + ni, sind);
                        }
                        t.p.records++;
                        t.p.reccount++;
                        $(t)
                            .triggerHandler("jqGridAfterInsertRow", [rowid, data, data]);
                        if (air) {
                ...
                $(this.p.colModel)
                    .each(function(i) {
                        if ($.inArray(this.name, colname) !== -1 && this.hidden === sw) {
                            ...
                            fndh = true;
                            $($t)
                                .triggerHandler("jqGridShowHideCol", [sw, this.name, i]);
                        }
                    });
                if (fndh === true) {
                    ...
                    $($t)
                        .jqGrid("setGridWidth", $t.p.shrinkToFit === true ? $t.p.tblwidth : $t.p.width);
                }
                if (gh) {
                    $($t)
                        .jqGrid('setGroupHeaders', $t.p.groupHeader);
                }
            });
        },
        ...
        remapColumns: function(permutation, updateCells, keepHeader) {
            ...
            $(ts)
                .triggerHandler("jqGridRemapColumns", [permutation, updateCells, keepHeader]);
        },
        setGridWidth: function(nwidth, shrink) {
            return this.each(function() {
                ...
                if ($t.p.tblwidth) {
                    $('table:first', $t.grid.bDiv)
                        .css("width", $t.p.tblwidth + "px");
                    $('table:first', $t.grid.hDiv)
                        .css("width", $t.p.tblwidth + "px");
                    $t.grid.hDiv.scrollLeft = $t.grid.bDiv.scrollLeft;
                    if ($t.p.footerrow) {
                        $('table:first', $t.grid.sDiv)
                            .css("width", $t.p.tblwidth + "px");
                    }
                }
            });
        ...
        setCaption: function(newcap) {
            return this.each(function() {
                ...
                $("span.ui-jqgrid-title, span.ui-jqgrid-title-rtl", this.grid.cDiv)
                    .html(newcap);
                $(this.grid.cDiv)
                    .show();
            });
        ...
        setCell: function(rowid, colname, nData, cssp, attrp, forceupd) {
            return this.each(function() {
                ...
                if (pos >= 0) {
                    //#85 - Sanitize value.
                    if (!$t.p.colModel[pos].skipsanitize)
                        nData = js.sanitizeString(nData);
                    var ind = $($t)
                        .jqGrid('getGridRowById', rowid);
                    if (ind) {
                        ...
                        if (nData !== "" || forceupd === true) {
                            ...
                            if ($t.p.treeGrid && $(".tree-wrap", $(tcell))
                                .length > 0) {
                                $("span", $(tcell))
                                    .html(v)
                                    .attr(title);
                ...
                if (pos >= 0) {
                    ...
                    if (ind) {
                        try {
                            ...
                            ret = $.jgrid.htmlDecode($("td:eq(" + pos + ")", ind)
                                .html());
                        }
                    }
                }
                    ...
                    if (ln && ln > 0) {
                        while (i < ln) {
                            if ($($t.rows[i])
                                ...
                                //Liveapp v4                                     
                                var selector = $($t.rows[i].cells[pos]);
                                var editor = js.findInput(selector);
                                if (editor) {
                                    val = editor.val();
                                }
        ...
        clearGridData: function(clearfooter) {
            return this.each(function() {
                ...
                if ($t.p.deepempty) {
                    $("#" + $.jgrid.jqID($t.p.id) + " tbody:first tr:gt(0)")
                        .remove();
                } else {
                        ...
                        if (target) {
                            ...
                            if (event.keyCode === 37 || event.keyCode === 38 || event.keyCode === 39 || event.keyCode === 40) {
                                ...
                                if (event.keyCode === 38) {
                                    ...
                                    $($t)
                                        .jqGrid('setSelection', id, true, event);
                                ...
                                if (event.keyCode === 37) {
                                    ...
                                    $($t)
                                        .triggerHandler("jqGridKeyLeft", [$t.p.selrow]);
                                ...
                                if (event.keyCode === 39) {
                                    ...
                                    $($t)
                                        .triggerHandler("jqGridKeyRight", [$t.p.selrow]);
                            ...
                            //check if enter was pressed on a grid or treegrid node
                                ...
                                $($t)
                                    .triggerHandler("jqGridKeyEnter", [$t.p.selrow]);
                                if ($.isFunction(o.onEnter)) {
                                    o.onEnter.call($t, $t.p.selrow);
                                }
                            } else if (event.keyCode === 32) {
                                $($t)
                                    .triggerHandler("jqGridKeySpace", [$t.p.selrow]);
    ...
    $.jgrid.extend({
        ...
        clearBeforeUnload: function() {
            return this.each(function() {
                ...
                $(document)
                    .unbind("mouseup.jqGrid" + this.p.id);
                $(grid.hDiv)
                    .unbind("mousemove"); // TODO add namespace
                $(this)
                    .unbind();
        ...
        GridDestroy: function() {
            return this.each(function() {
                if (this.grid) {
                    ...
                    try {
                        $(this)
                            .jqGrid('clearBeforeUnload');
                        $("#gbox_" + $.jgrid.jqID(this.id))
                            .remove();
                    } catch (_) {}
                }
            });
        ...
        setGridState: function(state) {
            return this.each(function() {
                ...
                var $t = this;
                if (state === 'hidden') {
                    $(".ui-jqgrid-bdiv, .ui-jqgrid-hdiv", "#gview_" + $.jgrid.jqID($t.p.id))
                        .slideUp("fast");
                    if ($t.p.pager) {
                        $($t.p.pager)
                            .slideUp("fast");
                    ...
                    if ($t.p.footerrow) {
                        $(".ui-jqgrid-sdiv", "#gbox_" + $.jgrid.jqID($t.p.id))
                            .slideUp("fast");
                    }
                    ...
                    $(".ui-jqgrid-hdiv, .ui-jqgrid-bdiv", "#gview_" + $.jgrid.jqID($t.p.id))
                        .slideDown("fast");
                    if ($t.p.pager) {
                        $($t.p.pager)
                            .slideDown("fast");
        ...
        filterToolbar: function(p) {
            ...
            return this.each(function() {
                ...
                var triggerToolbar = function() {
                        ...
                        var bsr = $($t)
                            .triggerHandler("jqGridToolbarBeforeSearch") === 'stop' ? true : false;
                        ...
                        $($t)
                            .triggerHandler("jqGridToolbarAfterSearch");
                    ...
                    clearToolbar = function(trigger) {
                        ...
                        var bcv = $($t)
                            .triggerHandler("jqGridToolbarBeforeClear") === 'stop' ? true : false;
                        ...
                        $("#sopt_menu > li > a")
                            ...
                            .click(function(e) {
                                var v = $(this)
                                    ...
                                    .attr("oper");
                                $($t)
                                    .triggerHandler("jqGridToolbarSelectOper", [v, oper, elem]);
                                $("#sopt_menu")
                                    .hide();
                                $(elem)
                                    .text(oper)
                ...
                $.each($t.p.colModel, function(ci) {
                    ...
                    if (this.search) {
                        ...
                        if (soptions.clearSearch) {
                            $("td:eq(2)", stbl)
                                .append("<a title='Clear Search Value' style='padding-right: 0.3em;padding-left: 0.3em;' class='clearsearchclass'>x</a>");
                        }
                        switch (this.stype) {
                            case "select":
                            ...
                            case "text":
                                ...
                                $("td:eq(1)", stbl)
                                    .append("<input type='text' style='width:100%;padding:0px;' name='" + (cm.index || cm.name) + "' id='gs_" + cm.name + "' value='" + df + "'/>");
                                $(thd)
                                    .append(stbl);
                            ...
                            case "custom":
                                $("td:eq(1)", stbl)
                                    .append("<span style='width:95%;padding:0px;' name='" + (cm.index || cm.name) + "' id='gs_" + cm.name + "'/>");
                                $(thd)
                                    .append(stbl);
                                try {
                ...
                $(".clearsearchclass", tr)
                    .click(function(e) {
                        var ptr = $(this)
                            .parents("tr:first"),
                            coli = parseInt($("td.ui-search-oper", ptr)
                                .attr('colindex'), 10),
                            sval = $.extend({}, $t.p.colModel[coli].searchoptions || {}),
                            dval = sval.defaultValue ? sval.defaultValue : "";
                        if ($t.p.colModel[coli].stype === "select") {
                            if (dval) {
                                $("td.ui-search-input select", ptr)
                                    .val(dval);
                            } else {
                                $("td.ui-search-input select", ptr)[0].selectedIndex = 0;
                            }
                        } else {
                            $("td.ui-search-input input", ptr)
                                .val(dval);
                        }
        ...
        setGroupHeaders: function(o) {
            ...
            return this.each(function() {
                ...
                var ts = this,
                    ...
                    $trLabels = $htable.children("thead")
                ...
                if (o.useColSpanStyle) {
                    ...
                    $htable.find("span.ui-jqgrid-resize")
                        .each(function() {
                            var $parent = $(this)
                                .parent();
                    ...
                    $htable.find("div.ui-jqgrid-sortable")
                        .each(function() {
                            var $ts = $(this),
                                $parent = $ts.parent();
                ...
                $(ts)
                    .bind('jqGridResizeStop.setGroupHeaders', function(e, nw, idx) {
                        $firstRow.find('th')
                            .eq(idx)
                            .width(nw);
                    });
            ...
            return this.each(function() {
                ...
                if (maxfrozen >= 0 && frozen) {
                    var top = $t.p.caption ? $($t.grid.cDiv)
                        .outerHeight() : 0,
                        hth = $(".ui-jqgrid-htable", "#gview_" + $.jgrid.jqID($t.p.id))
                        .height();
                    //headers
                    if ($t.p.toppager) {
                        top = top + $($t.grid.topDiv)
                            .outerHeight();
                    }
                    if ($t.p.toolbar[0] === true) {
                    ...
                    $("#gview_" + $.jgrid.jqID($t.p.id))
                        .append($t.grid.fhDiv);
                    var htbl = $(".ui-jqgrid-htable", "#gview_" + $.jgrid.jqID($t.p.id))
                        .clone(true);
                    // groupheader support - only if useColSpanstyle is false
                    if ($t.p.groupHeader) {
                        $("tr.jqg-first-row-header, tr.jqg-third-row-header", htbl)
                            .each(function() {
                                $("th:gt(" + maxfrozen + ")", this)
                                    .remove();
                            });
                        var swapfroz = -1,
                            fdel = -1,
                    ...
                    $($t)
                        .bind('jqGridResizeStop.setFrozenColumns', function(e, w, index) {
                            var rhth = $(".ui-jqgrid-htable", $t.grid.fhDiv);
                            $("th:eq(" + index + ")", rhth)
                                .width(w);
                            var btd = $(".ui-jqgrid-btable", $t.grid.fbDiv);
                            $("tr:first td:eq(" + index + ")", btd)
                                .width(w);
                        });
                    // sorting stuff
                    $($t)
                        .bind('jqGridSortCol.setFrozenColumns', function(e, index, idxcol) {
                            ...
                            $(previousSelectedTh)
                                .attr("aria-selected", "false");
                            $("span.ui-icon-" + $t.p.sortorder, newSelectedTh)
                                .removeClass('ui-state-disabled');
                            $(newSelectedTh)
                                .attr("aria-selected", "true");
                    ...
                    $($t)
                        .bind('jqGridAfterGridComplete.setFrozenColumns', function() {
                            $("#" + $.jgrid.jqID($t.p.id) + "_frozen")
                                .remove();
                            $($t.grid.fbDiv)
                                .height($($t.grid.bDiv)
                                    .height() - 16);
                            var btbl = $("#" + $.jgrid.jqID($t.p.id))
                                .clone(true);
                            $("tr[role=row]", btbl)
                                .each(function() {
                                    $("td[role=gridcell]:gt(" + maxfrozen + ")", this)
                                        .remove();
                                });
...
(function($) {
    ...
    $.jqm = {
        ...
        open: function(s, t) {
            ...
            h.o = (o) ? o.addClass(c.overlayClass)
                .prependTo('body') : F;
            ...
            if (c.toTop && h.o) h.w.before('<span id="jqmP' + h.w[0]._jqm + '"></span>')
                .insertAfter(h.o);
        ...
        close: function(s) {
            ...
            if (h.c.toTop && h.o) $('#jqmP' + h.w[0]._jqm)
                .after(h.w)
                .remove();
            ...
            var asXml = function(n) {
                ...
                if (n.nodeType === 1) {
                    ...
                    for (i = 0; i < n.attributes.length; i += 1) {
                        s += " " + n.attributes[i].nodeName + "=\"" + (n.attributes[i].nodeValue || "")
                            .toString() + "\"";
                    }
                    if (n.firstChild) {
                        s += ">";
...
(function($) {
    ...
    $.fn.fmatter.rowactions = function(act) {
        var $tr = $(this)
            ...
            $actionsDiv = cm.frozen ? $("tr#" + rid + " td:eq(" + $.jgrid.getCellIndex(this) + ") > div", $grid) : $(this)
            .parent(),
            op = {
                keys: false,
                onEdit: null,
            ...
            saverow = function(rowid, res) {
                ...
                $actionsDiv.find("div.ui-inline-edit,div.ui-inline-del")
                    .show();
    ...
    $.unformat = function(cellval, options, pos, cnt) {
        ...
        if (unformatFunc !== undefined && $.isFunction(unformatFunc)) {
            ret = unformatFunc.call(this, $(cellval)
                .text(), options, cellval);
            ...
            switch (formatType) {
                ...
                case 'currency':
                    ...
                    ret = ret.replace(stripTag, '')
                        .replace(op.decimalSeparator, '.');
                    break;
                case 'checkbox':
                    var cbv = (options.colModel.editoptions) ? options.colModel.editoptions.value.split(":") : ["Yes", "No"];
                    ret = $('input', cellval)
                        .is(":checked") ? cbv[0] : cbv[1];
                    break;
                case 'select':
        ...
        return ret !== undefined ? ret : cnt === true ? $(cellval)
            .text() : $.jgrid.htmlDecode($(cellval)
                .html());
    };
...
(function($) {
    ...
    $.extend($.jgrid, {
        ...
        createModal: function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
            ...
            $(mh)
                .append("<span class='ui-jqdialog-title'>" + p.caption + "</span>");
            ...
            $("a.ui-jqdialog-titlebar-close", mh)
                .click(function() {
                    var oncm = $("#" + $.jgrid.jqID(aIDs.themodal))
                        .data("onClose") || p.onClose;
                    var gboxclose = $("#" + $.jgrid.jqID(aIDs.themodal))
                        .data("gbox") || p.gbox;
            ...
            if (rtlsup && coord.left && !appendsel) {
                rtlt = $(p.gbox)
                    .width() - (!isNaN(p.width) ? parseInt(p.width, 10) : 0) - 8; // to do
                // just in case
            ...
            if (p.resize) {
                if ($.fn.jqResize) {
                    $(mw)
                        .append("<div class='jqResize ui-resizable-handle ui-resizable-se ui-icon ui-icon-gripsmall-diagonal-se'></div>");
                    $("#" + $.jgrid.jqID(aIDs.themodal))
                        .jqResize(".jqResize", aIDs.scrollelm ? "#" + $.jgrid.jqID(aIDs.scrollelm) : false);
                } else {
                    try {
                        $(mw)
                            .resizable({
            ...
            if (p.closeOnEscape === true) {
                $(mw)
                    .keydown(function(e) {
                        if (e.which == 27) {
                            var cone = $("#" + $.jgrid.jqID(aIDs.themodal))
                                .data("onClose") || p.onClose;
                            self.hideModal("#" + $.jgrid.jqID(aIDs.themodal), {
                                gb: p.gbox,
                                jqm: p.jqModal,
            ...
            $("#closedialog", "#info_id")
                .click(function() {
                    self.hideModal("#info_dialog", {
                        jqm: jm,
                        onClose: $("#info_dialog")
                            .data("onClose") || mopt.onClose,
                        gb: $("#info_dialog")
                            .data("gbox") || mopt.gbox
                    });
                    return false;
                });
            $(".fm-button", "#info_dialog")
                .hover(
                    function() {
                        $(this)
                            .addClass('ui-state-hover');
            ...
            if (opt.dataEvents) {
                $.each(opt.dataEvents, function() {
                    if (this.data !== undefined) {
                        $(el)
                            .bind(this.type, this.data, this.fn);
                    } else {
                        $(el)
            ...
            switch (eltype) {
                ...
                case "select":
                    ...
                    if (options.dataUrl !== undefined) {
                        var rowid = options.name ? String(options.id)
                            .substring(0, String(options.id)
                                .length - String(options.name)
                                .length - 1) : String(options.id),
                                    ...
                                    setTimeout(function() {
                                        $("option", elem)
                                            .each(function(i) {
                                                ...
                                                $(this)
                                                    .attr("role", "option");
                                                if ($.inArray($.trim($(this)
                                                        .text()), ovm) > -1 || $.inArray($.trim($(this)
                                                        .val()), ovm) > -1) {
...
(function($) {
    ...
    $.fn.jqFilter = function(arg) {
        ...
        return this.each(function() {
            ...
            if (this.p.showQuery) {
                $(this)
                    .append("<table class='queryresult ui-widget ui-widget-content' style='display:block;max-width:440px;border:0px none;' dir='" + this.p.direction + "'><tbody><tr><td class='query'></td></tr></tbody></table>");
            }
            ...
            */
            ...
            this.createTableRowForRule = function(rule, group) {
                ...
                ruleFieldSelect.bind('change', function() {
                    ...
                    $(".input-elm", trpar)
                        .bind('change', function(e) {
                            var tmo = $(this)
                                .hasClass("ui-autocomplete-input") ? 200 : 0;
                            setTimeout(function() {
                                var elem = e.target;
                                rule.data = elem.nodeName.toUpperCase() === "SPAN" && cm.searchoptions && $.isFunction(cm.searchoptions.custom_value) ?
                                    cm.searchoptions.custom_value.call($t, $(elem)
                                        .children(".customelement:first"), 'get') : elem.value;
                                that.onchange(); // signals that the filter has changed
                            }, tmo);
                        });
                ...
                $(ruleDataInput)
                    .addClass("input-elm")
                    .bind('change', function() {
                        rule.data = cm.inputtype === 'custom' ? cm.searchoptions.custom_value.call($t, $(this)
                                .children(".customelement:first"), 'get') : $(this)
                            .val();
                        that.onchange(); // signals that the filter has changed
                    });
    ...
    $.jgrid.extend({
        searchGrid: function(p) {
            ...
            return this.each(function() {
                ...
                function showFilter(_filter) {
                    showFrm = $($t)
                        .triggerHandler("jqGridFilterBeforeShow", [_filter]);
                    ...
                    if (showFrm) {
                        ...
                        $($t)
                            .triggerHandler("jqGridFilterAfterShow", [_filter]);
                ...
                if ($("#" + $.jgrid.jqID(IDs.themodal))[0] !== undefined) {
                    ...
                    $("#" + fid)
                        .jqFilter({
                            ...
                            onChange: function() {
                                if (this.p.showQuery) {
                                    $('.query', this)
                                        .html(this.toUserFriendlyString());
                                }
                    ...
                    if (found && p.tmplFilters && p.tmplFilters.length) {
                        $(".ui-template", fil)
                            .bind('change', function() {
                                ...
                                if (curtempl === "default") {
                                    $("#" + fid)
                                        .jqFilter('addFilter', defaultFilters);
                                } else {
                                    $("#" + fid)
                                        .jqFilter('addFilter', p.tmplFilters[parseInt(curtempl, 10)]);
                                }
                                return false;
                            });
                    }
                    ...
                    $($t)
                        .triggerHandler("jqGridFilterInitialize", [$("#" + fid)]);
                    ...
                    $("#" + fid + "_search")
                        .bind('click', function() {
                            ...
                            $.extend($t.p.postData, sdata);
                            mustReload = $($t)
                                .triggerHandler("jqGridFilterSearch");
                    ...
                    $("#" + fid + "_reset")
                        .bind('click', function() {
                            ...
                            $.extend($t.p.postData, sdata);
                            mustReload = $($t)
                                .triggerHandler("jqGridFilterReset");
                            if (mustReload === undefined) {
                                mustReload = true;
                            }
        ...
        editGridRow: function(rowid, p) {
            ...
            return this.each(function() {
                            ...
                            if (celm.length) {
                                ...
                                switch ($(this)
                                    ...
                                    case "checkbox":
                                        ...
                                        break;
                                    case "select-one":
                                        postdata[this.name] = $("option:selected", this)
                                            .val();
                                        break;
                            ...
                            if (nm !== 'cb' && nm !== 'subgrid' && this.editable === true && nm !== 'rn') {
                                if (ind === false) {
                                    ...
                                    if (nm === obj.p.ExpandColumn && obj.p.treeGrid === true) {
                                        tmp = $("td[role='gridcell']:eq(" + i + ")", obj.rows[ind])
                                            .text();
                                    } else {
                                        try {
                                            ...
                                            tmp = (this.edittype && this.edittype === "textarea") ? $("td[role='gridcell']:eq(" + i + ")", obj.rows[ind])
                                                .text() : $("td[role='gridcell']:eq(" + i + ")", obj.rows[ind])
                                                .html();
                                        }
                            ...
                            if (nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && cm[i].editable === true) {
                                if (nm === obj.p.ExpandColumn && obj.p.treeGrid === true) {
                                    ...
                                    try {
                                        ...
                                        tmp = cm[i].edittype === "textarea" ? $(this)
                                            .text() : $(this)
                                            .html();
                                    }
                                }
                                ...
                                switch (cm[i].edittype) {
                                    ...
                                    case "select":
                                        ...
                                        $("#" + nm + " option", "#" + fmid)
                                            .each(function() {
                                                if (!cm[i].editoptions.multiple && ($.trim(tmp) === $.trim($(this)
                                                        .text()) || opv[0] === $.trim($(this)
                                                        .text()) || opv[0] === $.trim($(this)
                                                        .val()))) {
                                                    this.selected = true;
                                                } else if (cm[i].editoptions.multiple) {
                                                    if ($.inArray($.trim($(this)
                                                            .text()), opv) > -1 || $.inArray($.trim($(this)
                                                            .val()), opv) > -1) {
                                                        this.selected = true;
                                                    } else {
                                                        this.selected = false;
                ...
                function postIt() {
                    ...
                    var retvals = $($t)
                        .triggerHandler("jqGridAddEditBeforeCheckValues", [$("#" + frmgr), frmoper]);
                    ...
                    if (ret[0]) {
                        onCS = $($t)
                            .triggerHandler("jqGridAddEditClickSubmit", [rp_ge[$t.p.id], postdata, frmoper]);
                        if (onCS === undefined && $.isFunction(rp_ge[$t.p.id].onclickSubmit)) {
                            onCS = rp_ge[$t.p.id].onclickSubmit.call($t, rp_ge[$t.p.id], postdata, frmoper) || {};
                        }
                        ret = $($t)
                            .triggerHandler("jqGridAddEditBeforeSubmit", [postdata, $("#" + frmgr), frmoper]);
                        if (ret === undefined) {
                            ret = [true, "", ""];
                        }
                    ...
                    if (ret[0] && !rp_ge[$t.p.id].processing) {
                        ...
                        var ajaxOptions = $.extend({
                            url: rp_ge[$t.p.id].url || $($t)
                                .jqGrid('getGridParam', 'editurl'),
                            ...
                            complete: function(data, status) {
                                ...
                                if (data.status >= 300 && data.status !== 304) {
                                    ret[0] = false;
                                    ret[1] = $($t)
                                        .triggerHandler("jqGridAddEditErrorTextFormat", [data, frmoper]);
                                    ...
                                    ret = $($t)
                                        .triggerHandler("jqGridAddEditAfterSubmit", [data, postdata, frmoper]);
                                    if (ret === undefined) {
                                        ret = [true, "", ""];
                                    }
                                ...
                                if (ret[0] === false) {
                                    ...
                                    if (postdata[oper] === opers.addoper) {
                                        ...
                                        if (rp_ge[$t.p.id].reloadAfterSubmit) {
                                            ...
                                            if ($t.p.treeGrid === true) {
                                                $($t)
                                                    .jqGrid("addChildNode", ret[2], selr, postdata);
                                            } else {
                                                $($t)
                                                    .jqGrid("addRowData", ret[2], postdata, p.addedrow);
                                            }
                                        }
                                        if (rp_ge[$t.p.id].closeAfterAdd) {
                                            if ($t.p.treeGrid !== true) {
                                                $($t)
                                                    .jqGrid("setSelection", ret[2]);
                                            }
                                            $.jgrid.hideModal("#" + $.jgrid.jqID(IDs.themodal), {
                                        ...
                                        if (rp_ge[$t.p.id].reloadAfterSubmit) {
                                            ...
                                            if (!rp_ge[$t.p.id].closeAfterEdit) {
                                                setTimeout(function() {
                                                    $($t)
                                                        .jqGrid("setSelection", postdata[idname]);
                                                }, 1000);
                                            }
                                        } else {
                                            if ($t.p.treeGrid === true) {
                                                $($t)
                                                    .jqGrid("setTreeRow", postdata[idname], postdata);
                                            } else {
                                                $($t)
                                                    .jqGrid("setRowData", postdata[idname], postdata);
                                            }
                                        }
                                        if (rp_ge[$t.p.id].closeAfterEdit) {
                                            $.jgrid.hideModal("#" + $.jgrid.jqID(IDs.themodal), {
                                    ...
                                    if ($.isFunction(rp_ge[$t.p.id].afterComplete)) {
                                        ...
                                        setTimeout(function() {
                                            $($t)
                                                .triggerHandler("jqGridAddEditAfterComplete", [copydata, postdata, $("#" + frmgr), frmoper]);
                ...
                function updateNav(cr, posarr) {
                    ...
                    if (cr === 0) {
                        $("#pData", frmtb + "_2")
                            .addClass('ui-state-disabled');
                    } else if (posarr[1][cr - 1] !== undefined && $("#" + $.jgrid.jqID(posarr[1][cr - 1]))
                        .hasClass('ui-state-disabled')) {
                        $("#pData", frmtb + "_2")
                            .addClass('ui-state-disabled');
                    } else {
                ...
                if ($("#" + $.jgrid.jqID(IDs.themodal))[0] !== undefined) {
                    showFrm = $($t)
                        .triggerHandler("jqGridAddEditBeforeInitData", [$("#" + $.jgrid.jqID(frmgr)), frmoper]);
                    if (showFrm === undefined) {
                        showFrm = true;
                    }
                    ...
                    restoreInline();
                    $(".ui-jqdialog-title", "#" + $.jgrid.jqID(IDs.modalhead))
                        .html(p.caption);
                    $("#FormError", frmtb)
                        .hide();
                    ...
                    if ($("#" + frmgr)
                        .data("disabled") === true) {
                        $(".confirm", "#" + $.jgrid.jqID(IDs.themodal))
                            .hide();
                        $("#" + frmgr)
                            .data("disabled", false);
                    }
                    $($t)
                        .triggerHandler("jqGridAddEditBeforeShowForm", [$("#" + frmgr), frmoper]);
                    if (onBeforeShow) {
                        onBeforeShow.call($t, $("#" + frmgr), frmoper);
                    }
                    $("#" + $.jgrid.jqID(IDs.themodal))
                        .data("onClose", rp_ge[$t.p.id].onClose);
                    $.jgrid.viewModal("#" + $.jgrid.jqID(IDs.themodal), {
                    ...
                    $($t)
                        .triggerHandler("jqGridAddEditAfterShowForm", [$("#" + frmgr), frmoper]);
                    if (onAfterShow) {
                    ...
                    var dh = isNaN(p.dataheight) ? p.dataheight : p.dataheight + "px",
                        ...
                        frm = $("<form name='FormPost' id='" + frmgr + "' class='FormGrid' onSubmit='return false;' style='width:" + dw + ";overflow:auto;position:relative;height:" + dh + ";'></form>")
                        .data("disabled", false),
                        tbl = $("<table id='" + frmtborg + "' class='EditTable' cellspacing='0' cellpadding='0' border='0'><tbody></tbody></table>");
                    showFrm = $($t)
                        .triggerHandler("jqGridAddEditBeforeInitData", [$("#" + frmgr), frmoper]);
                    if (showFrm === undefined) {
                        showFrm = true;
                    }
                    if (showFrm && onBeforeInit) {
                    ...
                    if (p.checkOnUpdate) {
                        $("a.ui-jqdialog-titlebar-close span", "#" + $.jgrid.jqID(IDs.themodal))
                    ...
                    if (rp_ge[$t.p.id].checkOnSubmit || rp_ge[$t.p.id].checkOnUpdate) {
                        ...
                        var zI = p.zIndex || 999;
                        zI++;
                        $("<div class='" + p.overlayClass + " jqgrid-overlay confirm' style='z-index:" + zI + ";display:none;'>&#160;" + "</div><div class='confirm ui-widget-content ui-jqconfirm' style='z-index:" + (zI + 1) + "'>" + p.saveData + "<br/><br/>" + bS + bN + bC + "</div>")
                            .insertAfter("#" + frmgr);
                    ...
                    $($t)
                        .triggerHandler("jqGridAddEditInitializeForm", [$("#" + frmgr), frmoper]);
                    if (onInitializeForm) {
                    ...
                    $(".fm-button", "#" + $.jgrid.jqID(IDs.themodal))
                        .hover(
                            function() {
                                $(this)
                                    .addClass('ui-state-hover');
                    ...
                    $("#nData", frmtb + "_2")
                        .click(function() {
                            ...
                            if (npos[0] !== -1 && npos[1][npos[0] + 1]) {
                                $($t)
                                    .triggerHandler("jqGridAddEditClickPgButtons", ['next', $("#" + frmgr), npos[1][npos[0]]]);
                                var nposret;
                                ...
                                if ($("#" + $.jgrid.jqID(npos[1][npos[0] + 1]))
                                    .hasClass('ui-state-disabled')) {
                                    return false;
                                }
                                fillData(npos[1][npos[0] + 1], $t, frmgr);
                                $($t)
                                    .jqGrid("setSelection", npos[1][npos[0] + 1]);
                                $($t)
                                    .triggerHandler("jqGridAddEditAfterClickPgButtons", ['next', $("#" + frmgr), npos[1][npos[0]]]);
                    ...
                    $("#pData", frmtb + "_2")
                        .click(function() {
                            ...
                            if (ppos[0] !== -1 && ppos[1][ppos[0] - 1]) {
                                $($t)
                                    .triggerHandler("jqGridAddEditClickPgButtons", ['prev', $("#" + frmgr), ppos[1][ppos[0]]]);
                                var pposret;
                                if ($.isFunction(p.onclickPgButtons)) {
                                ...
                                if ($("#" + $.jgrid.jqID(ppos[1][ppos[0] - 1]))
                                    .hasClass('ui-state-disabled')) {
                                    return false;
                                }
                                fillData(ppos[1][ppos[0] - 1], $t, frmgr);
                                $($t)
                                    .jqGrid("setSelection", ppos[1][ppos[0] - 1]);
                                $($t)
                                    .triggerHandler("jqGridAddEditAfterClickPgButtons", ['prev', $("#" + frmgr), ppos[1][ppos[0]]]);
                                if ($.isFunction(p.afterclickPgButtons)) {
        ...
        viewGridRow: function(rowid, p) {
            ...
            return this.each(function() {
                    ...
                    $(obj.p.colModel)
                        .each(function(i) {
                            ...
                            if (nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && viewfld) {
                                ...
                                $("td:eq(" + (cp - 2) + ")", trdata[0])
                                    .html('<b>' + (frmopt.label === undefined ? obj.p.colNames[i] : frmopt.label) + '</b>');
                                $("td:eq(" + (cp - 1) + ")", trdata[0])
                ...
                if ($("#" + $.jgrid.jqID(IDs.themodal))[0] !== undefined) {
                    ...
                    var bt = $("<div></div>")
                        .append(frm)
                        .append("<table border='0' class='EditTable' id='" + frmtb + "_2'><tbody><tr id='Act_Buttons'><td class='navButton' width='" + p.labelswidth + "'>" + (rtlb ? bN + bP : bP + bN) + "</td><td class='EditButton'>" + bC + "</td></tr></tbody></table>");
                ...
                if ($("#" + $.jgrid.jqID(IDs.themodal))[0] !== undefined) {
                    ...
                    $("#dData", "#" + dtbl + "_2")
                        .click(function() {
                            var ret = [true, ""],
                                pk,
                                postdata = $("#DelData>td", "#" + dtbl)
                                .text(); //the pair is name=val1,val2,...
                            onCS = {};
                                        ...
                                        if (ret[0] === false) {
                                            ...
                                            if (rp_ge[$t.p.id].reloadAfterSubmit && $t.p.datatype !== "local") {
                                                ...
                                                if ($t.p.treeGrid === true) {
                                                    try {
                                                        $($t)
                                                            .jqGrid("delTreeNode", $t.p.idPrefix + postdata[0]);
                                                    } catch (e) {}
                                                } else {
                                                    for (i = 0; i < postdata.length; i++) {
                                                        $($t)
                                                            .jqGrid("delRowData", $t.p.idPrefix + postdata[i]);
                                                    }
                                                }
                                                $t.p.selrow = null;
                                                $t.p.selarrrow = [];
            ...
            return this.each(function() {
                ...
                for (i = 0; i < clone; i++) {
                    ...
                    if (o.add) {
                        ...
                        $(tbd)
                            .append("<div class='ui-pg-div'><span class='ui-icon " + o.addicon + "'></span>" + o.addtext + "</div>");
                        $("tr", navtbl)
                            .append(tbd);
                        $(tbd, navtbl)
                            .attr({
                    ...
                    if (o.edit) {
                        ...
                        pEdit = pEdit || {};
                        $(tbd)
                            .append("<div class='ui-pg-div'><span class='ui-icon " + o.editicon + "'></span>" + o.edittext + "</div>");
                        $("tr", navtbl)
                            .append(tbd);
                        $(tbd, navtbl)
                            .attr({
                    ...
                    if (o.view) {
                        ...
                        pView = pView || {};
                        $(tbd)
                            .append("<div class='ui-pg-div'><span class='ui-icon " + o.viewicon + "'></span>" + o.viewtext + "</div>");
                        $("tr", navtbl)
                            .append(tbd);
                        $(tbd, navtbl)
                            .attr({
                    ...
                    if (o.del) {
                        ...
                        pDel = pDel || {};
                        $(tbd)
                            .append("<div class='ui-pg-div'><span class='ui-icon " + o.delicon + "'></span>" + o.deltext + "</div>");
                        $("tr", navtbl)
                            .append(tbd);
                        $(tbd, navtbl)
                            .attr({
                    ...
                    if (o.search) {
                        ...
                        pSearch = pSearch || {};
                        $(tbd)
                            .append("<div class='ui-pg-div'><span class='ui-icon " + o.searchicon + "'></span>" + o.searchtext + "</div>");
                        $("tr", navtbl)
                            .append(tbd);
                        $(tbd, navtbl)
                            .attr({
                    ...
                    if (o.refresh) {
                        tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
                        $(tbd)
                            .append("<div class='ui-pg-div'><span class='ui-icon " + o.refreshicon + "'></span>" + o.refreshtext + "</div>");
                        $("tr", navtbl)
                            .append(tbd);
                        $(tbd, navtbl)
                            .attr({
                    ...
                    tdw = $(".ui-jqgrid")
                        .css("font-size") || "11px";
                    $('body')
                        .append("<div id='testpg2' class='ui-jqgrid ui-widget ui-widget-content' style='font-size:" + tdw + ";visibility:hidden;' ></div>");
                    twd = $(navtbl)
                        .clone()
                        .appendTo("#testpg2")
                        .width();
        ...
        FormToGrid: function(rowid, formid, mode, position) {
            return this.each(function() {
                ...
                if (mode === 'add') {
                    $($t)
                        .jqGrid("addRowData", rowid, griddata, position);
                } else if (mode === 'set') {
                    $($t)
                        .jqGrid("setRowData", rowid, griddata);
                }
            });
        }
    });
    ...
    $.jgrid.extend({
            ...
            return this.each(function() {
                ...
                if (editable === "0" && !$(ind)
                        ...
                        .each(function(i) {
                            ...
                            if (treeg) {
                                ...
                                try {
                                    ...
                                    tmp = (cm[i].edittype && cm[i].edittype === 'textarea') ? $(this)
                                        .text() : $(this)
                                        .html();
                                }
                            }
                    ...
                    if (cnt > 0) {
                        ...
                        if (o.keys === true) {
                            $(ind)
                                .bind("keydown", function(e) {
                                    if (e.keyCode === 27) {
                                        $($t)
                                            .jqGrid("restoreRow", rowid, o.afterrestorefunc);
                                        if ($t.p._inlinenav) {
                                            try {
                                                $($t)
                                                    .jqGrid('showAddEditButtons');
                        ...
                        $($t)
                            .triggerHandler("jqGridInlineEditRow", [rowid, o]);
            ...
            if (editable === "1") {
                ...
                $('td[role="gridcell"]', ind)
                    .each(function(i) {
                        cm = $t.p.colModel[i];
                        nm = cm.name;
                        if (nm !== 'cb' && nm !== 'subgrid' && cm.editable === true && nm !== 'rn' && !$(this)
                ...
                if (o.url === 'clientArray') {
                    ...
                    $($t)
                        .triggerHandler("jqGridInlineAfterSaveRow", [rowid, resp, tmp, o]);
                    ...
                    $.ajax($.extend({
                        ...
                        complete: function(res, stat) {
                            ...
                            if (stat === "success") {
                                var ret = true,
                                    sucret, k;
                                sucret = $($t)
                                    .triggerHandler("jqGridInlineSuccessSaveRow", [res, rowid, o]);
                                if (!$.isArray(sucret)) {
                                    sucret = [true, tmp];
                                }
                                ...
                                if (ret === true) {
                                    ...
                                    $($t)
                                        .triggerHandler("jqGridInlineAfterSaveRow", [rowid, res, tmp, o]);
                                    if ($.isFunction(o.aftersavefunc)) {
                                    ...
                                    $($t)
                                        .triggerHandler("jqGridInlineErrorSaveRow", [rowid, res, stat, null, o]);
                        ...
                        error: function(res, stat, err) {
                            $("#lui_" + $.jgrid.jqID($t.p.id))
                                .hide();
                            $($t)
                                .triggerHandler("jqGridInlineErrorSaveRow", [rowid, res, stat, err, o]);
                            if ($.isFunction(o.errorfunc)) {
        ...
        restoreRow: function(rowid, afterrestorefunc) {
            ...
            return this.each(function() {
                ...
                if (fr >= 0) {
                    ...
                    $($t)
                        .jqGrid("setRowData", rowid, ares);
                    $(ind)
                        .attr("editable", "0")
                        .unbind("keydown");
                    $t.p.savedRow.splice(fr, 1);
                    if ($("#" + $.jgrid.jqID(rowid), "#" + $.jgrid.jqID($t.p.id))
                ...
                $($t)
                    .triggerHandler("jqGridInlineAfterRestoreRow", [rowid]);
        ...
        addRow: function(p) {
            ...
            return this.each(function() {
                ...
                $($t)
                    .jqGrid('addRowData', p.rowID, p.initdata, p.position);
                p.rowID = $t.p.idPrefix + p.rowID;
                $("#" + $.jgrid.jqID(p.rowID), "#" + $.jgrid.jqID($t.p.id))
                    .addClass("jqgrid-new-row");
                if (p.useFormatter) {
                    $("#" + $.jgrid.jqID(p.rowID) + " .ui-inline-edit", "#" + $.jgrid.jqID($t.p.id))
                        .click();
                } else {
                    ...
                    $($t)
                        .jqGrid('editRow', p.rowID, p.addRowParams);
                    $($t)
                        .jqGrid('setSelection', p.rowID);
                }
            });
        },
                    ...
                    $($t)
                        .jqGrid('navButtonAdd', elem, {
                            ...
                            onClickButton: function() {
                                ...
                                if (sr) {
                                    $($t)
                                        .jqGrid('editRow', sr, o.editParams);
                                    $("#" + gID + "_ilsave")
                                        .removeClass('ui-state-disabled');
                                    $("#" + gID + "_ilcancel")
                                        .removeClass('ui-state-disabled');
                        ...
                        .jqGrid('navButtonAdd', elem, {
                            ...
                            onClickButton: function() {
                                ...
                                if (sr) {
                                    ...
                                    $($t)
                                        .jqGrid('restoreRow', sr, cancelPrm);
                                    $($t)
                                        .jqGrid('showAddEditButtons');
                                } else {
                                    $.jgrid.viewModal("#alertmod", {
                    ...
                    $t.p.beforeSelectRow = function(id, stat) {
                        ...
                        if ($t.p.savedRow.length > 0 && $t.p._inlinenav === true && (id !== $t.p.selrow && $t.p.selrow !== null)) {
                            if ($t.p.selrow === o.addParams.rowID) {
                                $($t)
                                    .jqGrid('delRowData', $t.p.selrow);
                            } else {
                                $($t)
                                    .jqGrid('restoreRow', $t.p.selrow, o.editParams);
                            }
                            $($t)
                                .jqGrid('showAddEditButtons');
                        }
        ...
        editCell: function(iRow, iCol, ed) {
            return this.each(function() {
                ...
                if ($t.p.savedRow.length > 0) {
                    ...
                    // save the cell
                    $($t)
                        .jqGrid("saveCell", $t.p.savedRow[0].id, $t.p.savedRow[0].ic);
                } else {
                ...
                if (cm.editable === true && ed === true && !cc.hasClass("not-editable-cell")) {
                    ...
                    try {
                        ...
                        tmp = (cm.edittype && cm.edittype === 'textarea') ? $(cc)
                            .text() : $(cc)
                            .html();
                    }
                    if ($t.p.autoencode) {
                    ...
                    $($t)
                        .triggerHandler("jqGridBeforeEditCell", [$t.rows[iRow].id, nm, tmp, iRow, iCol]);
                    ...
                    $("input, select, textarea", cc)
                        .bind("keydown", function(e) {
                            ...
                            if (e.keyCode === 13) {
                                ...
                                //return false; Liveapp v4
                                if (!$t.grid.hDiv.loading) {
                                    if (e.shiftKey) {
                                        $($t)
                                            .jqGrid("prevCell", iRow, iCol);
                                    } //Shift TAb
                                    else {
                                        $($t)
                                            .jqGrid("nextCell", iRow, iCol);
                                    } //Tab
                                } else {
                                    return false;
                                }
                    ...
                    $($t)
                        .triggerHandler("jqGridAfterEditCell", [$t.rows[iRow].id, nm, tmp, iRow, iCol]);
                    ...
                    tmp = cc.html()
                        .replace(/\&#160\;/ig, '');
                    $($t)
                        .triggerHandler("jqGridSelectCell", [$t.rows[iRow].id, nm, tmp, iRow, iCol]);
            ...
            return this.each(function() {
                ...
                if (fr !== null) {
                    ...
                    switch (cm.edittype) {
                        case "select":
                            if (!cm.editoptions.multiple) {
                                v = $("#" + iRow + "_" + nmjq + " option:selected", $t.rows[iRow])
                                    .val();
                                v2 = $("#" + iRow + "_" + nmjq + " option:selected", $t.rows[iRow])
                                    .text();
                            } else {
                    ...
                    if (v2 !== $t.p.savedRow[fr].v) {
                        var vvv = $($t)
                            .triggerHandler("jqGridBeforeSaveCell", [$t.rows[iRow].id, nm, v, iRow, iCol]);
                        if (vvv) {
                            v = vvv;
                            v2 = vvv;
                        }
                        ...
                        if (cv[0] === true) {
                            var addpost = $($t)
                                .triggerHandler("jqGridBeforeSubmitCell", [$t.rows[iRow].id, nm, v, iRow, iCol]) || {};
                                ...
                                if ($t.p.cellurl) {
                                    ...
                                    $.ajax($.extend({
                                        ...
                                        complete: function(result, stat) {
                                            ...
                                            if (stat === 'success') {
                                                var ret = $($t)
                                                    .triggerHandler("jqGridAfterSubmitCell", [$t, result, postdata.id, nm, v, iRow, iCol]) || [true, ''];
                                                ...
                                                if (ret[0] === true) {
                                                    $(cc)
                                                        .empty();
                                                    $($t)
                                                        .jqGrid("setCell", $t.rows[iRow].id, iCol, v2, false, false, true);
                                                    $(cc)
                                                        .addClass("dirty-cell");
                                                    $($t.rows[iRow])
                                                        .addClass("edited");
                                                    $($t)
                                                        .triggerHandler("jqGridAfterSaveCell", [$t.rows[iRow].id, nm, v, iRow, iCol]);
                                        ...
                                        error: function(res, stat, err) {
                                            ...
                                            $t.grid.hDiv.loading = false;
                                            $($t)
                                                .triggerHandler("jqGridErrorCell", [res, stat, err]);
            ...
            return this.each(function() {
                ...
                if (fr !== null) {
                    ...
                    $(cc)
                        ...
                        .attr("tabindex", "-1");
                    $($t)
                        .jqGrid("setCell", $t.rows[iRow].id, iCol, $t.p.savedRow[fr].v, false, false, true);
                    $($t)
                        .triggerHandler("jqGridAfterRestoreCell", [$t.rows[iRow].id, $t.p.savedRow[fr].v, iRow, iCol]);
        ...
        nextCell: function(iRow, iCol) {
            return this.each(function() {
                ...
                for (i = iCol + 1; i < $t.p.colModel.length; i++) {
                    if ($t.p.colModel[i].editable === true && $t.p.colModel[i].hidden != true) { //Liveapp v4
                        nCol = i;
                        break;
                    }
                }
                if (nCol !== false) {
                    $($t)
                        .jqGrid("editCell", iRow, nCol, true);
                } else {
                    if ($t.p.savedRow.length > 0) {
                        $($t)
                        ...
                        switch (kdir) {
                            case 38:
                                if ($t.p.iRow - 1 > 0) {
                                    ...
                                    $($t)
                                        .jqGrid("editCell", $t.p.iRow - 1, $t.p.iCol, false);
                                }
                                break;
                            case 40:
                            ...
                            case 37:
                                if ($t.p.iCol - 1 >= 0) {
                                    ...
                                    $($t)
                                        .jqGrid("editCell", $t.p.iRow, i, false);
                                }
                                break;
                            case 39:
                            ...
                            case 13:
                                if (parseInt($t.p.iCol, 10) >= 0 && parseInt($t.p.iRow, 10) >= 0) {
                                    $($t)
                                        .jqGrid("editCell", $t.p.iRow, $t.p.iCol, true);
                                }
                                break;
                                //Liveapp v4
                            case 9:
        ...
        addSubGrid: function(pos, sind) {
            return this.each(function() {
                ...
                var subGridCell = function(trdiv, cell, pos) {
                    var tddiv = $("<td align='" + ts.p.subGridModel[0].align[pos] + "'></td>")
                        .html(cell);
                    $(trdiv)
                        .append(tddiv);
                };
                var subGridXml = function(sjxml, sbid) {
                    ...
                    for (i = 0; i < ts.p.subGridModel[0].name.length; i++) {
                        ...
                        $(tddiv)
                            .html(ts.p.subGridModel[0].name[i]);
                        $(tddiv)
                        ...
                        $(ts.rows[i].cells[pos])
                            .bind('click', function() {
                                ...
                                if ($(this)
                                    ...
                                    pID = ts.p.id;
                                    _id = tr.id;
                                    if (ts.p.subGridOptions.reloadOnExpand === true || (ts.p.subGridOptions.reloadOnExpand === false && !$(r)
                                            .hasClass('ui-subgrid'))) {
                                        atd = pos >= 1 ? "<td colspan='" + pos + "'>&#160;</td>" : "";
                                        bfsc = $(ts)
                                            .triggerHandler("jqGridSubGridBeforeExpand", [pID + "_" + _id, _id]);
                                        ...
                                        $(tr)
                                            .after("<tr role='row' class='ui-subgrid'>" + atd + "<td class='ui-widget-content subgrid-cell'><span class='ui-icon " + ts.p.subGridOptions.openicon + "'></span></td><td colspan='" + parseInt(ts.p.colNames.length - 1 - nhc, 10) + "' class='ui-widget-content subgrid-data'><div id=" + pID + "_" + _id + " class='tablediv'></div></td></tr>");
                                        $(ts)
                                            .triggerHandler("jqGridSubGridRowExpanded", [pID + "_" + _id, _id]);
                                    ...
                                    $(this)
                                        .html("<a style='cursor:pointer;'><span class='ui-icon " + ts.p.subGridOptions.minusicon + "'></span></a>")
                                        .removeClass("sgcollapsed")
                                        .addClass("sgexpanded");
                                    if (ts.p.subGridOptions.selectOnExpand) {
                                        $(ts)
                                            .jqGrid('setSelection', _id);
                                    }
                                } else if ($(this)
                                    .hasClass("sgexpanded")) {
                                    bfsc = $(ts)
                                        .triggerHandler("jqGridSubGridRowColapsed", [pID + "_" + _id, _id]);
                                    bfsc = (bfsc === false || bfsc === 'stop') ? false : true;
                                    _id = tr.id;
                                    ...
                                    if (ts.p.subGridOptions.reloadOnExpand === true) {
                                        ...
                                        .hasClass('ui-subgrid')) { // incase of dynamic deleting
                                        $(r)
                                            .hide();
                                    }
                                    $(this)
                                        .html("<a style='cursor:pointer;'><span class='ui-icon " + ts.p.subGridOptions.plusicon + "'></span></a>")
    ...
    $.jgrid.extend({
                    ...
                    $($t.rows[i].cells[expCol])
                        ...
                        .bind("click", function(e) {
                            var target = e.target || e.srcElement,
                                ind2 = $.jgrid.stripPref($t.p.idPrefix, $(target, $t.rows)
                                    .closest("tr.jqgrow")[0].id),
                                pos = $t.p._index[ind2];
                            if (!$t.p.data[pos][isLeaf]) {
                                if ($t.p.data[pos][expanded]) {
                                    $($t)
                                        .jqGrid("collapseRow", $t.p.data[pos]);
                                    $($t)
                                        .jqGrid("collapseNode", $t.p.data[pos]);
                                } else {
                                    $($t)
                                        .jqGrid("expandRow", $t.p.data[pos]);
                                    $($t)
                                        .jqGrid("expandNode", $t.p.data[pos]);
                                }
                            }
                            return false;
                        });
            ...
            this.each(function() {
                ...
                $(childern)
                    .each(function() {
                        ...
                        $($($t)
                                .jqGrid('getGridRowById', id))
                            .css("display", "");
                        if (this[expanded]) {
                            $($t)
                                .jqGrid("expandRow", this);
                        }
                    });
                //}
            });
            ...
            this.each(function() {
                ...
                switch ($t.p.treeGridModel) {
                    ...
                    case 'adjacency':
                        ...
                        $($t.p.data)
                            .each(function() {
                                if (this[parent_id] === null || String(this[parent_id])
                                    .toLowerCase() === "null") {
                                    result.push(this);
                                }
                            });
        ...
        getNodeAncestors: function(rc) {
            ...
            this.each(function() {
                ...
                var parent = $(this)
                    .jqGrid("getNodeParent", rc);
                while (parent) {
                    ancestors.push(parent);
                    parent = $(this)
                        .jqGrid("getNodeParent", parent);
                }
            });
            return ancestors;
        },
        ...
        expandNode: function(rc) {
            return this.each(function() {
                ...
                if (!rc[expanded]) {
                    ...
                    if ($(this)
                        .jqGrid("isNodeLoaded", this.p.data[position])) {
                        rc[expanded] = true;
                        $("div.treeclick", rc1)
        ...
        SortTree: function(sortname, newDir, st, datefmt) {
            return this.each(function() {
                ...
                for (i = 0, len = roots.length; i < len; i++) {
                    ...
                    records.push(rec);
                    $(this)
                        .jqGrid("collectChildrenSortTree", records, rec, sortname, newDir, st, datefmt);
                }
        ...
        collectChildrenSortTree: function(records, rec, sortname, newDir, st, datefmt) {
            return this.each(function() {
                ...
                for (i = 0, len = children.length; i < len; i++) {
                    ...
                    records.push(child);
                    $(this)
                        .jqGrid("collectChildrenSortTree", records, child, sortname, newDir, st, datefmt);
                }
            });
        },
        // experimental 
        ...
        delTreeNode: function(rowid) {
            return this.each(function() {
                ...
                if (rc !== undefined) {
                    ...
                    var dr = $($t)
                        .jqGrid("getFullTreeNode", $t.p.data[rc]);
                    if (dr.length > 0) {
                        for (i = 0; i < dr.length; i++) {
                            $($t)
                                .jqGrid("delRowData", dr[i][rid]);
                        }
                    }
                    if ($t.p.treeGridModel === "nested") {
        ...
        addChildNode: function(nodeid, parentid, data, expandData) {
            ...
            if (data) {
                ...
                if (parentid === undefined || parentid === null || parentid === "") {
                    ...
                    var childs = $($t)
                        .jqGrid('getFullTreeNode', parentdata);
                    // if there are child nodes get the last index of it
                    if (childs.length) {
                        i = childs[childs.length - 1][$t.p.localReader.id];
                        rowind = i;
                        i = $($t)
                            .jqGrid('getInd', rowind) + 1;
                    } else {
                        i = $($t)
                            .jqGrid('getInd', parentid) + 1;
                    }
                    // if the node is leaf
                ...
                if ($t.p.treeGridModel === "nested") {
                    ...
                    if (parentid !== null) {
                        ...
                        maxright = parseInt($($t)
                            .jqGrid('getCol', right, false, 'max'), 10);
                        res = $.jgrid.from($t.p.data)
                ...
                if (parentid === null || $($t)
                    .jqGrid("isNodeLoaded", parentdata) || leaf) {
                    $($t)
                        .jqGrid('addRowData', nodeid, data, method, rowind);
                    $($t)
                        .jqGrid('setTreeNode', i, len);
                }
                    ...
                    if (!grp.groupField.length) {
                        ...
                        for (i = 0; i < grp.groupField.length; i++) {
                            ...
                            if (grp.groupColumnShow[i] === true) {
                                ...
                                grp.visibiltyOnNextGrouping[i] = $("#" + $.jgrid.jqID($t.p.id + "_" + grp.groupField[i]))
                                    .is(":visible");
                                $($t)
        ...
        groupingPrepare: function(rData, gdata, record, irow) {
            this.each(function() {
                ...
                for (i = 0; i < grlen; i++) {
                    ...
                    fieldCaption = grp.groupFieldCaption[i];
                    v = record[fieldName];
                            ...
                            $.each(grp.counters[i].summary, function() {
                                if ($.isFunction(this.st)) {
                                    ...
                                    if (this.st.toLowerCase() === 'avg' && this.sd) {
                                        this.vd = $($t)
                                            .jqGrid('groupingCalculations.handler', this.st, this.vd, this.sd, this.sr, this.srt, record);
                                    }
                                }
                            });
        ...
        groupingToggle: function(hid) {
            this.each(function() {
                ...
                if (tarspan.hasClass(minus)) {
                    ...
                    tarspan.removeClass(plus)
                        .addClass(minus);
                }
                $($t)
                    .triggerHandler("jqGridGroupingClickGroup", [hid, collapsed]);
        ...
        groupingRender: function(grdata, colspans) {
            return this.each(function() {
                ...
                $.each(grp.groups, function(i, n) {
                    ...
                    str += "<tr id=\"" + hid + "\"" + (grp.groupCollapse && n.idx > 0 ? " style=\"display:none;\" " : " ") + "role=\"row\" class= \"ui-widget-content jqgroup ui-row-" + $t.p.direction + " " + clid + "\"><td style=\"padding-left:" + (n.idx * 12) + "px;" + "\" colspan=\"" + colspans + "\">" + icon + $.jgrid.template(grp.groupText[n.idx], (gv == null ? 'blank' : gv), n.cnt, n.summary, n.fieldCaption) + "</td></tr>";
                    ...
                    if (leaf) {
                        ...
                        for (ik = 0; ik < toEnd; ik++) {
                            ...
                            var gv2 = null;
                            try {
                                if ($.isArray(grp.formatDisplayField) && $.isFunction(grp.formatDisplayField[fdata.idx])) {
                                    fdata.displayValue = grp.formatDisplayField[fdata.idx].call($t, fdata.displayValue, fdata.value, $t.p.colModel[cp[fdata.idx]], fdata.idx, grp);
                                }
                                gv2 = $t.formatter(hid, fdata.displayValue, cp[fdata.idx], fdata.value);
                            } catch (eee) {
                                gv2 = fdata.displayValue;
                            }
                            str += "<td style=\"padding-left:" + (fdata.idx * 12) + "px;" + "\" colspan=\"" + $t.p.groupingView.colspan + "\">Total " + $.jgrid.template(grp.groupText[fdata.idx], (gv2 == null ? 'blank' : gv2), fdata.cnt, fdata.summary, fdata.fieldCaption) + "</td>";

                            for (k = parseInt($t.p.groupingView.colspan); k < colspans; k++) {
        ...
        groupingGroupBy: function(name, options) {
            return this.each(function() {
                ...
                for (i = 0; i < name.length; i++) {
                    grp.visibiltyOnNextGrouping[i] = $("#" + $.jgrid.jqID($t.p.id) + "_" + $.jgrid.jqID(name[i]))
                        .is(":visible");
                }
            ...
            return this.each(function() {
                ...
                if (current === true) {
                    ...
                    for (i = 0; i < grp.groupField.length; i++) {
                        if (!grp.groupColumnShow[i] && grp.visibiltyOnNextGrouping[i]) {
                            $($t)
                                .jqGrid('showCol', grp.groupField);
                        }
                    }
                    $("tr.jqgroup, tr.jqfoot", "#" + $.jgrid.jqID($t.p.id) + " tbody:first")
                        .remove();
                    $("tr.jqgrow:hidden", "#" + $.jgrid.jqID($t.p.id) + " tbody:first")
                        .show();
                } else {
                    $($t)
                        .trigger("reloadGrid");
                ...
                switch (o.imptype) {
                        ...
                        $.ajax($.extend({
                            ...
                            complete: function(xml, stat) {
                                if (stat === 'success') {
                                    ...
                                    $($t)
                                        .triggerHandler("jqGridImportComplete", [xml, o]);
                    ...
                    case 'xmlstring':
                        ...
                        if (o.impstring && typeof o.impstring === 'string') {
                            ...
                            if (xmld) {
                                xmlConvert(xmld, o);
                                $($t)
                                    .triggerHandler("jqGridImportComplete", [xmld, o]);
                                if ($.isFunction(o.importComplete)) {
                        ...
                        $.ajax($.extend({
                            ...
                            complete: function(json) {
                                try {
                                    ...
                                    $($t)
                                        .triggerHandler("jqGridImportComplete", [json, o]);
                                    if ($.isFunction(o.importComplete)) {
                    ...
                    case 'jsonstring':
                        if (o.impstring && typeof o.impstring === 'string') {
                            ...
                            $($t)
                                .triggerHandler("jqGridImportComplete", [o.impstring, o]);
                            if ($.isFunction(o.importComplete)) {
...
(function($) {
    ...
    $.jgrid.extend({
            ...
            return this.each(function() {
                ...
                var sortable_opts = {
                    ...
                    "update": function(event, ui) {
                        ...
                        $(ts)
                            .jqGrid("remapColumns", permutation, true, true);
        ...
        columnChooser: function(opts) {
            var self = this;
            if ($("#colchooser_" + $.jgrid.jqID(self[0].p.id))
                .length) {
                return;
            }
            ...
            function call(fn, obj) {
                ...
                if (typeof fn === 'string') {
                    ...
                    fn.apply(obj, $.makeArray(arguments)
                        .slice(2));
                }
            }
                ...
                if ($.fn.sortable) {
                    ...
                    opts.start = function(ev, ui) {
                        ...
                        if ($t.p.subGrid) {
                            ...
                            try {
                                $($t)
                                    .jqGrid('collapseSubGridRow', subgid);
                            } catch (e) {}
                        }
                        if (opts._start_) {
                    ...
                    opts.update = function(ev, ui) {
                        ...
                        if ($t.p.rownumbers === true) {
                            $("td.jqgrid-rownum", $t.rows)
                                .each(function(i) {
                                    $(this)
                                        .html(i + 1 + (parseInt($t.p.page, 10) - 1) * parseInt($t.p.rowNum, 10));
                                });
                        }
                        if (opts._update_) {
            ...
            return this.each(function() {
                ...
                function updateDnD() {
                    ...
                    $("tr.jqgrow:not(.ui-draggable)", $t)
                        .draggable($.isFunction(datadnd.drag) ? datadnd.drag.call($($t), datadnd) : datadnd.drag);
                }
                    ...
                    "drag": function(opts) {
                        return $.extend({
                            start: function(ev, ui) {
                                ...
                                for (i = 0; i < $.data($t, "dnd")
                                    .connectWith.length; i++) {
                                    if ($($.data($t, "dnd")
                                            .connectWith[i])
                            ...
                            stop: function(ev, ui) {
                                ...
                                for (i = 0; i < $.data($t, "dnd")
                                    ...
                                    $($.data($t, "dnd")
                                            .connectWith[i])
                                        .jqGrid('delRowData', 'jqg_empty_row');
                                }
                    ...
                    "drop": function(opts) {
                        return $.extend({
                            ...
                            drop: function(ev, ui) {
                                ...
                                var accept = $(ui.draggable)
                                    .attr("id");
                                var getdata = ui.draggable.parent()
                                    .parent()
                ...
                for (i = 0; i < opts.connectWith.length; i++) {
                    ...
                    $(cn)
                        .droppable($.isFunction(opts.drop) ? opts.drop.call($($t), opts) : opts.drop);
                }
            });
        },
        gridResize: function(opts) {
            return this.each(function() {
                ...
                opts.stop = function(ev, ui) {
                    ...
                    $($t)
                        .jqGrid('setGridWidth', ui.size.width, opts.shrinkToFit);
                    if (opts._stop_) {
    ...
    jQuery(selector)
        .each(function() {
            ...
            jQuery('th', jQuery(this))
                .each(function() {
                    if (colModel.length === 0 && selectable) {
                        ...
                        colModel.push({
                            name: jQuery(this)
                                .attr("id") || jQuery.trim(jQuery.jgrid.stripHtml(jQuery(this)
                                    .html()))
                                .split(' ')
                                .join('_'),
                            index: jQuery(this)
            ...
            for (a = 0; a < data.length; a++) {
                ...
                jQuery(this)
                    .jqGrid("addRowData", id, data[a]);
            }

            // Set the selection
            for (a = 0; a < rowChecked.length; a++) {
                jQuery(this)
                    .jqGrid("setSelection", rowChecked[a]);
            }
        });
};