...
backbrace.controller('api', function(viewer) {
    ...
    page.template = '<h1 class="api-heading">{{name}}</h1>{{githubLinks}}' +
        '<label class="api-type {{kind}}">{{kind}}</label>' +
    ...
    function convertLink(val, links) {
        ...
        $.each(vals, function(index, v) {
            ...
            if (links.indexOf(v) === -1) {
                ...
                ret.push('<a style="color:#1abc9c;text-decoration:underline;" route="api/' + v + '">' + encv + '</a>');
            }
        });
        return ret.join('|');
    }
            ...
            return backbrace.promiseblock(
                ...
                function(members) {
                    ...
                    members = $.grep(members, function(val) {
                        ...
                        return isMember || isProperty || isConstruct;
                    }).map(function(val) {
                        val.sort = 1;
